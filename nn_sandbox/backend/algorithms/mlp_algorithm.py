...
class MlpAlgorithm(PredictiveAlgorithm):
    ...
    def _initialize_neurons(self, shape):
        ...
        self._output_neuron = self._neurons[-1][0]
    ...
    def _pass_backward(self, expect):
        """ Calculate the delta for each neuron. """
        deltas = {}

        deltas[self._output_neuron] = ((expect - self._output_neuron.result)
                                       * self._output_neuron.result
                                       * (1 - self._output_neuron.result))
    ...
    def _correct_rate(self, dataset):
        ...
        for data in dataset:
            ...
            if expect - interval < self._output_neuron.result < expect + interval:
                correct_count += 1
        if correct_count == 0:
            return 0
